...
// links with:  gcc -nostdlib -no-pie -static out.s -o prog
//
// Build:  gcc -std=c11 -O2 -Wall -Wextra -o nano_cc 'simpleC++.c'
// Use:    ./nano_cc input.c -o output.s
// =====================================================================
...
static void err_message(srcloc_t loc, const char *kind, const char *fmt, va_list ap) {
    const char *filename = get_filename(loc);
    if (filename) fprintf(stderr, "%s:%d: %s: ", filename, get_lineno(loc), kind);
...
static void warning(srcloc_t loc, const char *fmt, ...) attr_printf(2,3);
static void warning(srcloc_t loc, const char *fmt, ...) {
    va_list a; va_start(a, fmt); err_message(loc, "warning", fmt, a); va_end(a);
}
static void note(srcloc_t loc, const char *fmt, ...) attr_printf(2,3);
static void note(srcloc_t loc, const char *fmt, ...) {
    va_list a; va_start(a, fmt); err_message(loc, "note", fmt, a); va_end(a);
}
static void error(Node *n, const char *fmt, ...) attr_printf(2,3);

static unsigned char xdigit(unsigned char d) {
...
static size_t skip_pp_number(const char *p) {
    const char *q = p;
    for (unsigned char c, last = 0; (c = (unsigned char)*p++); last = c) {
        if (c == '.' && *p != '.') continue;
...
static size_t pstrcat(char *dest, size_t size, const char *src) {
...
static size_t encode_string(char *buf, size_t size, const char *str, size_t slen, char sep) {
    ...
    size_t j = 0;
#define PUT(c)  (void)(j < size && (buf[j++] = (char)(c)))
    if (sep) PUT(sep);
...
static int counter;    // value of the last expansion of __COUNTER__
...
#define LINE_LEN 8192   // max line length for preprocessing
...
static void expand_line(const char *in, bool preproc, sbuf_t *sb, char *dest, size_t size) {
    char work1[LINE_LEN], work2[LINE_LEN];
    const char *p = in;
    for (int pass = 0;; pass++) {
        const char *q = p;
        char *out = (p == work1) ? work2 : work1;
        size_t j = 0; bool changed = false;
        ...
        while ((c = *p) != '\0') {
            ...
            if (isalpha((unsigned char)c) || c == '_') {
                ...
                j += macro_expand(m, &ma, def, out + j, LINE_LEN - j);
                changed = true;
            } else {
                p++;
            }
        ...
        if (j >= LINE_LEN) die("macro expansion overflow");
        // should complain about recursion
        if (changed && pass < 8) { out[j] = '\0'; p = out; continue; } // swap buffers
        if (sb) {
            sbuf_put(sb, q, j);
            return;
        } else {
            pmemcpy(dest, size, q, j);
            if (j < size) dest[j] = 0;
            return;
        }
    }
...
static size_t strip_comments(char *s) {
    ...
    for (size_t k;;) {
        switch (c = *q = *p++) {
        case '\0': return (size_t)(q - s);
        case '\n':
            q -= trailing_blanks(s, (size_t)(q - s)); need_lines++;
            while (need_lines) { *q++ = '\n'; need_lines--; }
            bol = true; continue;
        case ' ': case '\t':
            ...
            if (*p == '*') {
                while (*++p) {
                    ...
                    if (*p == '\n') {    // preserve line numbers
                        q -= trailing_blanks(s, (size_t)(q - s));
                        *q++ = '\n'; bol = true;
                    }
                }
                if (!bol && q[-1] != ' ') *q++ = ' ';
...
static void process_text(const char *text, sbuf_t *sb) {
    ...
    while (*src) {
        ...
        if (*p == '#') {
            ...
            p = skip_blanks(p + di);
            switch (dir) {
            case K_IFDEF: case K_IFNDEF:
                cond_sp++; seen_true <<= 1; seen_else <<= 1; skip <<= 1;
                if (!skip) {
                    atom_t nm = parse_macro_name(dir, &p);
                    int defined = (atom_flags(nm) & ATOM_MACRO) != 0;
                    if ((dir == K_IFDEF) == defined) {
                        seen_true |= 1;
                    } else {
                        skip |= 1;
                    }
            ...
            case K_IF:
                ...
                break;
            case K_ELIF:
                check_cond_else(dir, cond_sp, seen_else);
                skip &= ~1UL;
                skip |= seen_true & 1;
                if (!skip) {
                    if (eval_preproc(p)) {
            ...
            case K_ELIFNDEF:
                ...
                if (!skip) {
                    atom_t nm = parse_macro_name(dir, &p);
                    size_t defined = (atom_flags(nm) & ATOM_MACRO) != 0;
                    if ((dir == K_ELIFDEF) == defined) {
                        seen_true |= 1;
                    } else {
                        skip |= 1;
                    }
            ...
            case K_ELSE:
                ...
                break;
            case K_ENDIF:
                check_cond_else(dir, cond_sp, 0);
                cond_sp--;
                seen_else >>= 1; seen_true >>= 1; skip >>= 1;
                break;
            case K_DEFINE: {
                if (skip) break;
            ...
            case K_UNDEF: {
                if (skip) break;
                macro_undef(parse_macro_name(dir, &p));
                break;
            }
            case K_LINE:
                if (skip) break;
            ...
            if (!skip) expand_line(line, false, sb, NULL, 0);
        }
        sbuf_putc(sb, '\n'); src_loc++; // preserve line numbers
    }
    if (cond_sp) warning(src_loc, "missing '#endif'");
}
...
static int read_escape(const char *p, size_t *len) { // `p` points just past a backslash
    *len = 1;
    ...
    case 'x':
        c = 0;
        unsigned char d;
        while ((d = xdigit((unsigned char)*p++)) < 16) { c = c * 16 + d; *len += 1; }
        return (unsigned char)c;
    default:
        warning(src_loc, "invalid escape sequence '\\%c'", c);
        return (unsigned char)c;
    }
}

static unsigned char const ops[] = {
#define ____ 0
    '(', T_LP,       ____,      ____,         ____,
    ')', T_RP,       ____,      ____,         ____,
...
static size_t lex(const char *p) {
    ...
    for (;;) {
        ...
        Token *t = &toks[ntok++]; t->loc = src_loc;

#define set_tok(k)  t->kind = (unsigned char)(k)
        if (!c) {
            p--; set_tok(T_EOF);
        ...
        if (isdigit(c)) {
            ...
            // should ignore '
            while ((d = xdigit(c = (unsigned char)*p)) < base) { v = v * base + d; p++; }
            t->base = base;
            t->uval = v;
        ...
        has_float:;
            char c1 = c | 0x20;
            if (c == '.' || c1 == 'e' || c1 == 'p' || c1 == 'f') {
                warning(t->loc, "floating point not supported");
        ...
        if (isalpha(c) || c == '_') {
            ...
            if (IS_KEYWORD(name)) { set_tok(name); continue; }
        ...
        if (c == '#') { // parse # number [filename]
            ...
            while (isdigit((unsigned char)*p)) lineno = lineno * 10 + (unsigned)(*p++ - '0');
            p = skip_blanks(p);
            if (*p == '"') {
                size_t k = skip_until(++p, '"');
                filename = atom_str(new_atom_len(p, k)); p += k;
            }
            src_loc = update_loc(src_loc, filename, lineno);
            p += skip_until(p, '\n');
            p += (*p == '\n');
...
typedef enum TypeKind enum_type(unsigned char) {
    ...
    TY_CHAR, TY_ENUM,
    TY_VOID, TY_PTR, TY_ARRAY, TY_STRUCT, TY_UNION, TY_FUNC,
} TypeKind;
static const char * const type_name[] = {
    "int", "signed char", "short", "long",
    "unsigned int", "unsigned char", "unsigned short", "unsigned long",
    "char", "enum", "void", "ptr", "array", "struct", "union", "func",
};
...
struct Member {
    atom_t name; srcloc_t loc;
    unsigned char align, bf_width, bf_shift;
    unsigned offset, pad;
...
static Member *find_member(Member *m, atom_t name) {
    for (; m; m = m->next) {
        if (m->name == name) break;
        if (!m->name) {
            Type *mt = m->type;
            if (mt->kind == TY_STRUCT || mt->kind == TY_UNION) {
                Member *mm = find_member(mt->members, name);
                if (mm) return mm;
            }
        }
    }
...
static Member *find_struct_member(Type *st, atom_t name, Node *n) {
    if (st->kind == TY_PTR) st = st->base;
    Member *m = find_member(st->members, name);
    if (!m && n) error(n, "struct member '%s' not found", atom_str(name));
    return m;
}
...
// ---- symbols ----
typedef enum SymKind enum_type(unsigned char) {
    S_NONE, S_GLOBAL, S_AUTO, S_STATIC_LOCAL, S_REGISTER, S_TYPEDEF,
    S_ENUM_VALUE,
} SymKind;
...
static Sym *sym_link(Sym *s) {
    ...
    return atom_sym(s->name) = s;
}
static Sym *sym_unlink(Sym *s) {
    return atom_sym(s->name) = s->next_sym;
}

static void set_local_offset(Sym *s) {
    if (s->kind != S_AUTO) return;
...
static Sym *new_sym(atom_t name, srcloc_t loc, SymKind kind, Type *type, unsigned int sflags) {
    Sym *s = allocz(1, sizeof(Sym));
    s->kind = kind; s->sflags = (unsigned char)sflags;
    s->name = name; s->loc = loc; s->type = type;
    if (kind == S_STATIC_LOCAL) s->num = static_num++;
    return s;
}
static Sym *scope_add_sym(Sym *s) {
    if (scope_depth) {
        s->scope_depth = scope_depth;
        if (s->kind == S_AUTO) set_local_offset(s); // should delay this until analysis to allocate registers
        if (scope_len == scope_cap) scope_list = reallocate(scope_list, &scope_cap, sizeof(Sym*), 64);
        return scope_list[scope_len++] = sym_link(s);
    } else {
        if (globals) globals_tail->next_decl = s; else globals = s;
        return globals_tail = sym_link(s);
    }
}

static Sym *add_sym(atom_t name, srcloc_t loc, Type *type, unsigned int sflags) {
    SymKind kind = S_GLOBAL;
    if (sflags & HAS_TYPEDEF) kind = S_TYPEDEF;
    else if (scope_depth) {
        ...
        case 0:           kind = S_AUTO; break;
        case HAS_STATIC:  kind = S_STATIC_LOCAL; break;
        default:          break;
        }
    }
    Sym *s = new_sym(name, loc, kind, type, sflags);
    return scope_add_sym(s);
}
...
static Node *scope_pop(Node *n) {
    while (scope_len > n->scope_len) {
        ...
        sym_unlink(s);
        if (s->kind == S_AUTO) frame_pos = s->offset;
    }
...
static Sym *lookup(atom_t name, Node *n) {
    ...
    else if (s->kind == S_STATIC_LOCAL) error(n, "static local definitions not supported");
    return s;
}
static Type *find_typedef(atom_t name) {
    Sym *s = lookup(name, NULL);
    if (s && s->kind == S_TYPEDEF) return s->type;
    switch (name) {
...
static bool same_type(Type *t1, Type *t2) {
    ...
    if (t1 && t2) {
        ...
        case TY_FUNC:
            ...
            ||  t1->np != t2->np
            ||  !same_type(t1->rtype, t2->rtype)) return false;
            for (Sym *p1 = t1->params, *p2 = t2->params; p1; p1 = p1->next_decl, p2 = p2->next_decl) {
                if (!same_type(p1->type, p2->type)) return false;
            }
            return true;
        default: break;
        }
...
#define HAS_FRAME  4  // function has a standard frame
    ...
    Sym *params; Node *body; Type *rtype; Type *type; Node *labels;
    struct Func *next;
};
...
static Sym *parse_function(Sym *s, unsigned int flags);
static Type *parse_type_suffix(Type *base, unsigned char fflags);
...
static Type *parse_type_suffix(Type *base, unsigned char fflags) {
    if (eat(T_LBRK)) base = parse_array(base, fflags);
    else if (eat(T_LP)) { // parse function parameters
        Type *t = allocz(1, sizeof(Type));
        t->kind = TY_FUNC; t->align = 1;
        t->rtype = base;
        Sym **pp = &t->params;
        if (at(K_VOID) && peek(T_RP)) P++; // (void) -> ()
        while (!at(T_RP)) {
            if (eat(T_ELLIPSIS)) { t->is_variadic = true; break; }   // printf(char *fmt, ...)
            if (t->np >= MAX_ARGS) error(NULL, "too many function arguments");
            parse_attributes();
            unsigned int pflags;
            atom_t pname = 0;
            srcloc_t ploc = curloc();
            Type *t1 = parse_declaration_specifiers(&pflags);
            Type *pt = parse_declarator(t1, &pname, &ploc, &pflags, IS_FUN_ARG);
            // no linking!
            Sym *pv = new_sym(pname, ploc, S_AUTO, pt, pflags);
            *pp = pv; pp = &pv->next_decl; t->np++;
    ...
    if (eat(T_LBRACE)) {                           // definition
        ...
        while (!at(T_RBRACE) && !at(T_EOF)) {
            ...
            for (;;) {
                atom_t name = 0;
                srcloc_t mloc = 0;
                Type *mt = parse_declarator(base, &name, &mloc, &flags, 0);
                if (name) {
                    Member *mm = find_member(head, name);
                    if (mm) {
                        warning(mloc, "redefinition of member '%s'", atom_str(name));
                        note(mm->loc, "previous definition of '%s' is here", atom_str(name));
                    }
                    if (mt->kind == TY_FUNC) {
                        //if (at(T_LBRACE)) parse_function(s, flags);
                        error(NULL, "member functions not supported yet");
                    }
                } else
                if (!at(T_COLON)) {
        ...
        if (kind == TY_UNION) {
            ...
            if (tail) tail->pad = (unsigned)(st->struct_size - off);
        }
    }
    return st;
}
...
static Type *parse_enum(void) {
    ...
    if (eat(T_COLON)) {
        ...
        if (et->kind >= TY_ENUM) error(NULL, "invalid enum underlying type");
    }
    Type *st = tag_get(tag, TY_ENUM, loc);
    if (st->enum_type) {
        if (at(T_LBRACE)) warning(loc, "enum type already defined");
        if (et && st->enum_type != et) warning(loc, "enum type inconsistent with previous declaration");
    } else {
        st->enum_type = et;
    }
    if (eat(T_LBRACE)) {  // definition
        ...
        while (!at(T_RBRACE) && !at(T_EOF)) {
            srcloc_t ploc = curloc();
            atom_t name = getid();
            Sym *s = new_sym(name, ploc, S_ENUM_VALUE, st, 0);
            scope_add_sym(s);
            parse_attributes();
            if (eat(T_ASSIGN)) {
...
static Type *parse_type(void) {
    unsigned int flags;
    Type *base = parse_declaration_specifiers(&flags);
    // XXX: should reject function types
...
static bool eval_expr(Node *n, Value *vp) {
    ...
    if (!value_check_range(&v1, n->type)) {
        ...
        if (n->type->kind == TY_PTR) {
            ...
            warning(n->loc, "%s is too large for type %s", buf, type_str(n->type));
        }
    }
...
static long eval_preproc(const char *p) {
    srcloc_t save_loc = src_loc;
    char buf[LINE_LEN];
    expand_line(p, true, NULL, buf, countof(buf));
    lex(buf);
    P = toks;
    Node *e = parse_expr();
    Value v;
    eval_const_expr(e, &v, "preprocessor test expression");
    // should reset node pool
    ntok = 0;
    src_loc = save_loc;
    return !!v.uval;
}
...
static Sym *parse_function(Sym *s, unsigned int flags) {
    Func *fn = s->fn = allocz(1, sizeof(Func));
    Type *t = s->type;
    fn->name = s->name;
    fn->type = t;
    fn->rtype = t->rtype;
    ...
    Node fun = {};
    scope_push(&fun);
    for (Sym *s1 = t->params; s1; s1 = s1->next_decl) {
        // XXX check for shadowing
        scope_add_sym(s1);
    }

    fn->is_variadic = t->is_variadic;
    fn->nparams = t->np;
    fn->params = t->params;
...
static Node *parse_declaration(void) {
    ...
    for (;;) {
        ...
        if (!name) { warning(curloc(), "unnamed declaration"); goto next; }
        for (Sym *s = atom_sym(name); s; s = s->next_sym) {
            bool is_same_type = same_type(s->type, t);
            if (is_same_type) {
                if (s->kind == S_TYPEDEF && (flags & HAS_TYPEDEF)) continue;
                if ((flags & HAS_EXTERN) || t->kind == TY_FUNC
                ||  (s->sflags & HAS_EXTERN) || s->type->kind == TY_FUNC) {
                    continue;
                }
            }
            if (s->scope_depth == scope_depth) {
                warning(nloc, "redefinition of symbol '%s'", atom_str(name)); // this is an error
            } else {
                if (get_fileflags(nloc)) break;
                warning(nloc, "symbol '%s' shadows previous definition", atom_str(name));
            }
            note(s->loc, "previous definition of '%s' is here", atom_str(name));
        }
        Sym *s = add_sym(name, nloc, t, flags);
        if (t->kind == TY_FUNC) {  // only accept function prototype
            if (at(T_LBRACE)) {
                parse_function(s, flags);
                if (scope_depth) warning(nloc, "local functions are not supported yet");
                break;
            }
        } else {
            ...
            if (eat(T_ASSIGN)) {
                ...
                if (init->kind == N_BLOCK && t->kind == TY_ARRAY && !(t->pflags & HAS_LEN)) {
                    ...
                    if (s->kind == S_AUTO) set_local_offset(s);
                }
            }
        }
...
static int emit_indent(int col, int indent) {
    ...
    if (!no_tabs) {
        for (int next; (next = (col + 8) & ~7) <= indent; col = next) putc('\t', fout);
    }
    while (col < indent) { putc(' ', fout); col++; }
    return col;
}
...
static void emit(const char *fmt, ...) {
    int col = 0;
    if (next_label) {
        if (next_label >= 10) { fputc('.', fout); fputc('L', fout); col = 2; }
        col += fprintf(fout, "%d:", next_label);
        next_label = 0;
    }
    if (*fmt != ' ') {  // no empty format strings to avoid gcc warning
        col = emit_indent(col, 8);
...
static void emit_entry(atom_t name, unsigned char align, bool globl, bool skip) {
    if (!name) return;
...
static void emit_mov_reg_reg(Register r1, Register r2) {
    if (r1 != r2) emit_reg_reg("mov", r1, r2);
}
...
static const char *make_address(char *dest, size_t size, Sym *s, unsigned offset, srcloc_t loc) {
    switch (s->kind) {
    case S_GLOBAL:   if (offset) snprintf(dest, size, "[rip + \"%s\" + %u]", atom_str(s->name), offset);
                     else        snprintf(dest, size, "[rip + \"%s\"]", atom_str(s->name));
                     return dest;
    case S_AUTO:                 snprintf(dest, size, "[rbp - %u]", s->offset - offset); return dest;
    case S_STATIC_LOCAL: if (offset) snprintf(dest, size, "[rip + \"%s_%u\" + %u]", atom_str(s->name), s->num, offset);
                     else        snprintf(dest, size, "[rip + \"%s_%u\"]", atom_str(s->name), s->num);
                     return dest;
    case S_REGISTER: return reg64[s->reg];
    case S_TYPEDEF:
    case S_ENUM_VALUE:
...
static Type *gen_addr(Node *n, Register r, bool save_rax, unsigned offset) {
    ...
    case N_VAR: {
        ...
        case S_GLOBAL:
        case S_AUTO:
        case S_STATIC_LOCAL: emit("lea %s, %s", reg64[r], src); break;
        case S_REGISTER:
        case S_TYPEDEF:
        case S_ENUM_VALUE:
    ...
    case N_MEMBER: {   // &(s.field)
        ...
        Member *m = find_struct_member(st, n->name, n);
        emit_comment(atom_str(n->name));
        if (m->offset) { emit_reg_imm("add", r, m->offset); }
        return m->type;
    }
    default:
...
static void emit_imul_imm(Register r, long val) {
    ...
    case 3:
    case 5:
    case 9:  emit("lea %s, [%s+%ld*%s]", reg, reg, val - 1, reg); return;
    default:
        if ((val & (val - 1)) == 0) { // power of 2
            emit_reg_imm("shl", r, __builtin_ctzl((unsigned long)val));
            return;
        } else {
...
static Type *gen_bin(Node *n, TokenKind op, Type *lt, Type *rt, Register r) {
    // rax = left, rcx = right, r = dest
    ...
    case T_PLUS:
        ...
        if (r == RCX) { emit("add rcx, rax"); return ct; }
        emit("add rax, rcx");
        break;
    case T_MINUS:
        if (is_ptrish(lt)) {
    ...
    case T_PERCENT:
        if (ty_is_unsigned(ct)) {
            emit_reg_reg("xor", RDX, RDX); emit("div rcx");
        } else {
            emit("cqo"); emit("idiv rcx");
        }
        if (op == T_PERCENT) { emit_mov_reg_reg(r, RDX); return ct; }
        break;
    case T_AMP:     emit("and rax, rcx"); break;
    case T_BITOR:   emit("or rax, rcx");  break;
    case T_BITXOR:  emit("xor rax, rcx"); break;
    case T_SHL:     emit("shl rax, cl"); ct = lt; break;   // count in cl (low byte of rcx)
    case T_SHR:     if (ty_is_unsigned(lt)) emit("shr rax, cl"); else emit("sar rax, cl");  ct = lt; break;
...
static Type *load_var(Node *n, Register r) {
    ...
    switch (t->kind) {
    case TY_FUNC:
        emit("lea %s, %s", reg, src);
        break;
    case TY_ARRAY: case TY_STRUCT: case TY_UNION:  // arrays and structs are used by-address (decay); scalars are loaded
        switch (s->kind) {
        case S_GLOBAL:
        case S_AUTO:
        case S_STATIC_LOCAL: emit("lea %s, %s", reg, src); break;
        case S_REGISTER: error(n, "invalid type for a register variable"); break;
        case S_TYPEDEF:
        case S_ENUM_VALUE:
        default:         break;  // error
        }
    ...
    default: {
        ...
        case S_REGISTER:
            ...
            break;
        case S_TYPEDEF:  // error
        case S_ENUM_VALUE:  // never here, should be resolved as a constant already
        default:     break;
        }
        break;
    }}
...
static Type *store_var(Sym *s, srcloc_t loc, unsigned offset, Type *t, Register r) {
    ...
    case TY_STRUCT: case TY_UNION:
        ...
        break;
    default:
        if (s->kind != S_GLOBAL) emit_comment(atom_str(s->name));
        if (s->kind == S_REGISTER) { emit("mov %s, %s", dest, reg64[r]); break; }
...
static Type *gen_expr(Node *n, Register r, bool save_rax) {
    ...
    case N_MEMBER: {
        if (n->lhs->kind == N_DEREF) { // optimize p->member -> *((*p).member)
            ...
            Member *m = find_struct_member(lt, n->name, n);
            emit_comment(atom_str(n->name));
            Type *mt = m->type;
    ...
    case N_ASSIGN: {
        ...
        if (!(n->flags & DISCARD)) promote_reg(lt, RAX);
        emit_mov_reg_reg(r, RAX);
        if (save_rax) emit("pop rax");
        return lt;
    }
    ...
    case N_BUILTIN: {
        ...
        case ID__BUILTIN_CLZ:
        case ID__BUILTIN_CLZL:
            gen_expr(lhs, r, save_rax); emit_reg_reg("bsr", r, r); emit_reg_imm("xor", r, 63);
            return ty_int();
        case ID__BUILTIN_CTZ:
        case ID__BUILTIN_CTZL:
            gen_expr(lhs, r, save_rax); emit_reg_reg("rep bsf", r, r);
            return ty_int();
        }
        return ty_void();
    }
    case N_CALL: {
        ...
        case ID__RDTSCP:
            check_num_args(n, 0);
            if (save_rax) emit("push rax");
            if (n->name == ID__RDTSCP) emit("rdtscp"); else emit("rdtsc");
            emit("shl rdx, 32"); emit("or rax, rdx");
            goto done_pop_rax;
        done_pop_rax:
            emit_mov_reg_reg(r, RAX);
            if (save_rax) emit("pop rax");
        ...
        if (!fn && n->lhs) {
            // indirect call: n->lhs should be a pointer to function
            // XXX: should get the type to check arguments
            Type *t = gen_expr(n->lhs, RAX, false);
            if (t->kind == TY_PTR) ft = t->base;
            if (!ft || ft->kind != TY_FUNC) {
                error(n, "call expression is not a function type");
            }
            emit("push rax");
        }
        ...
        if (all_simple_load(n)) {
            ...
            for (Node *arg = n->rhs; arg; arg = arg->next, i++) {
                ...
                else { gen_expr(arg, RAX, false); emit_mov_reg_reg(ARGREG[i], RAX); }
                break;
            }
        ...
        emit_mov_reg_reg(r, RAX);
        if (save_rax) emit("pop rax");
        if (ft) return ft->rtype;
        warning(n->loc, "function not found '%s'", atom_str(n->name));
        return ty_long();
    }
    case N_COMMA:
    ...
    case N_CMP:
        if (n->rhs->flags & CONST_VAL) {
            ...
            Type *rt = gen_expr(n->rhs, RCX, true);
            lt = gen_bin(n, n->op, lt, rt, r);
            // should cast to common type
            if (save_rax) emit("pop rax");
            return lt;
        }
    ...
    for (Node *e = n->lhs; e; e = e->lhs) {
        ...
        if (t2) {
            ...
            if (!value_check_range(&v1, ct) || !value_check_range(&v2, ct)) {
                warning(e->cond->loc, "'case' range expressions are inconsistent with condition type");
                e->lab = DEST_NONE;
            }
            if (expr_less(t2, t1, ct)) {
                warning(e->loc, "'case' range is empty");
                e->lab = DEST_NONE;
            }
        } else {
            if (!value_check_range(&v1, ct)) {
                warning(e->cond->loc, "'case' expression is beyond the range of condition type");
                e->lab = DEST_NONE;
            }
            value_cast(&v1, ct);
        }
...
static Flow gen_stmt(Node *n) {
    ...
    case N_DECL:
        for (Sym *s = n->decl; s; s = s->next_decl) {
            ...
            if ((e = s->init)) {
                if (e->kind == N_BLOCK) {
                    gen_init(s->type, s->name, e, NULL, s, 0);
                } else {
...
static void gen_func(Func *fn) {
    ...
    if (body->rhs && body->rhs->kind == N_ASM && !body->rhs->next) {
...
// return the index of the last non zero initializer plus 1
static unsigned check_zero_init(Node *e) {
    unsigned n = 0;
    for (unsigned i = 0; e; i++, e = e->next) {
        if (e->kind == N_BLOCK) {
            if (!check_zero_init(e->rhs)) continue;
        } else
        if ((e->flags & CONST_VAL) && !e->uval) continue;
        n = i + 1;
    }
    return n;
}
static void gen_init_zero(Sym *s, unsigned offset, unsigned size) {
    if (s) {
        store_var_zero(s, s->loc, offset, size);
        return;
    }
    emit(".zero %u", size);
}
...
static Node *get_member_initializer(Type *t, Node *n, Member *m1) {
    ...
    for (; n; n = n->next) {
        if (n->kind == N_DESIGNATOR) {
            ...
            if (!m || n->name != m->name) {
                m = find_struct_member(t, n->name, NULL);
            }
        }
        if (m == m1) found = n;
        if (m) m = m->next;
...
static void gen_init_struct(Type *t, Node *init, Sym *s, unsigned offset) {
    if (init->flags & HAS_DESIGNATOR) {
        if (t->kind == TY_UNION) {
            ...
            for (; n; n = n->next) {
                if (n->kind == N_DESIGNATOR) {
                    Member *m1 = find_struct_member(t, n->name, NULL);
    ...
    if (init) {
        ...
        case TY_CHAR: case TY_ENUM:
            ...
            if (eval_const_expr(init, &v, "initializer")) {
                ...
                default: warning(t->loc, "invalid size %u for type %d", ty_size(t), t->kind); exit(1);
                }
            }
            break;
        case TY_PTR:
...
static int emit_x86_intel(bool kernel_mode, bool libc_mode) {
    ...
    emit_section(".data", 0);
    for (Sym *s = globals; s; s = s->next_decl) {
        if (s->kind != S_GLOBAL) continue;
        if (s->fn) continue;
        if (s->type->kind == TY_FUNC) continue;
        ...
        if (!kernel_mode && !s->init) {
            ...
            emit(".comm %s, %u", atom_str(s->name), sz);
        } else {
            emit_entry(s->name, s->type->align, !(s->sflags & HAS_STATIC), false);
            gen_init(s->type, s->name, s->init, NULL, NULL, 0);
        }
    }
