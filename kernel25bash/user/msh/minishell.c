...
static void var_unset(const char *name) {
    ...
    /* PORT: the comma operator in a for-increment is outside nano_cc's
       subset. The loop always breaks on a match, so moving the advance to
       the bottom of the body is the same loop. */
    Var *v = variables;
    while (v) {
...
static void free_node(Node *n) {
    ...
    switch (n->type) {
        ...
        case N_IF: {
            If *ifn = n->data;
            free_list_tree(ifn->cond);
            free_list_tree(ifn->then);
            free_node(ifn->else_node);
            free(ifn);
            break;
        }
        ...
        case N_FOR: {
            For *forn = n->data;
            free(forn->var);
            for (int i = 0; i < forn->nwords; i++) free(forn->words[i]);
            free(forn->words);
            free_list_tree(forn->body);
            free(forn);
            break;
        }
...
static char *command_substitute(const char *text) {
    ...
    /* PORT: command substitution runs the shell's own evaluator in a child,
       so it needs a real fork -- there is no program to start. Named rather
       than reported as a generic fork failure, because the parser accepts
       $(...) and the user would otherwise think the command inside was at
       fault. */
    pid_t pid = fork();
    if (pid < 0) {
        fprintf(stderr, "minishell: $(...) and `...` need fork, which this "
                        "build does not have\n");
        close(fds[0]);
        close(fds[1]);
        return xstrdup("");
...
static int expand_dollar(Exp *e, const char *s, int i, int inquote) {
    ...
    if (s[i] == '{') {
        ...
        if (sub && (strcmp(sub, "@") == 0 || strcmp(sub, "*") == 0)) {
            ...
            Var *avar = var_find(name);
            if (avar) {
                for (int k = 0; k < avar->nitems; k++) {
                    if (!avar->items[k]) continue;
                    ...
                    if (bang) {
                        ...
                        args_add_copy(&items, avar->items[k]);
                    }
                }
            } else if (!bang) {
        ...
        if (hashlen) {
            ...
            if (op == '#') {
                for (size_t k_p1 = 0; k_p1 <= vlen; k_p1++) {
                    char save = ((char *)vstr)[k_p1];
                    char *tmp = xmalloc(k_p1 + 1);
                    memcpy(tmp, vstr, k_p1);
                    tmp[k_p1] = '\0';
                    (void)save;

                    if (glob_match(pat, tmp)) {
                        if (!found || (colon ? k_p1 > best : k_p1 < best)) {
                            best = k_p1;
                            found = 1;
                        }
                ...
                exp_addstr(e, found ? vstr + best : vstr, m);
            } else {
                for (size_t k_p2 = 0; k_p2 <= vlen; k_p2++) {
                    size_t cut = colon ? k_p2 : vlen - k_p2;
                    if (glob_match(pat, vstr + cut)) {
                        best = cut;
                        found = 1;
                        break;
                    }
...
/*
 * PORT: redirection without dup2.
 *
 * On Unix the child is forked first and then rearranges its own descriptors.
 * This kernel has no fork and no exec: a process's descriptors are fixed by
 * the SPAWN that creates it, which takes the child's stdout and stdin as
 * arguments. So a redirection has to be RESOLVED BEFORE the child exists,
 * which is what this does -- open the files, and hand back the two numbers
 * spawn wants.
 *
 * What that costs, stated plainly rather than discovered later:
 *   - only fd 0 and fd 1 can be redirected. `2>file` has nowhere to go,
 *     because spawn has no third argument.
 *   - `>&` and `<&` duplicate one descriptor onto another, which is dup2 by
 *     definition and cannot be expressed this way at all.
 *   - here-documents need a second process to write the body, so they belong
 *     with the fork work.
 * Each of those reports itself by name. The parser accepts all of them, so a
 * silent failure would look like the user's script was wrong.
 */
static int msh_redirs_to_fds(Redir *rs, int n, char **files,
                             int *outfd, int *infd) {
    *outfd = -1;
    *infd = -1;
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (r->type == T_DLESS) {
            /*
             * A here-document does NOT need fork after all, as long as the
             * body fits in the pipe. miniShell already had this path: it
             * writes the body straight into the pipe and only forks a writer
             * when the body is large. The bound here is the KERNEL's pipe
             * buffer, not the 32768 the original assumed -- write one byte
             * more than the buffer holds with no reader yet started and the
             * shell blocks forever waiting for a drain that cannot happen.
             * So the limit is checked, and refused rather than hung.
             */
            int hfds[2];
            long blen = (long)strlen(files[i]);

            if (blen > MSH_PIPE_CAP) {
                fprintf(stderr, "minishell: here-document is %ld bytes; the "
                                "pipe holds %d and there is no fork to write "
                                "the rest\n", blen, MSH_PIPE_CAP);
                return 0;
            }
            if (r->fd != 0) {
                fprintf(stderr, "minishell: a here-document can only feed fd 0 "
                                "in this build (asked for fd %d)\n", r->fd);
                return 0;
            }
            if (pipe(hfds) < 0) {
                fprintf(stderr, "minishell: pipe: out of pipes\n");
                return 0;
            }
            if (blen > 0 && write(hfds[1], files[i], blen) != blen) {
                fprintf(stderr, "minishell: here-document: short write\n");
                close(hfds[0]); close(hfds[1]);
                return 0;
            }
            /* Closing the write end is what gives the reader its EOF. */
            close(hfds[1]);
        ...
        if (r->type == T_GREATAND || r->type == T_LESSAND) {
            fprintf(stderr, "minishell: >& and <& need dup2, which this build "
                            "does not have\n");
            return 0;
        }
        if (r->fd != 0 && r->fd != 1) {
            fprintf(stderr, "minishell: only fd 0 and fd 1 can be redirected "
                            "in this build (asked for fd %d)\n", r->fd);
            return 0;
        }
        ...
        if (fd < 0) {
            fprintf(stderr, "minishell: %s: cannot open\n", files[i]);
            return 0;
        }

        /* A later redirection of the same stream wins, as in any shell. */
        if (r->fd == 0) { if (*infd  >= 0) close(*infd);  *infd  = fd; }
        else            { if (*outfd >= 0) close(*outfd); *outfd = fd; }
    }
    return 1;
}
 ...
 * PORT: pipeline state.
 *
 * execute_pipeline's child used to call execute_node -- the shell's own
 * evaluator -- which is why it needed a real fork. Rather than duplicate all
 * of the expansion and redirection logic into a second path, the pipeline
 * sets these and lets the ORDINARY simple-command path do the work: it spawns
 * with the pipe's descriptors and, in pipeline mode, does not wait. The
 * pipeline collects the pids and waits for all of them at the end, so the
 * stages really do run at the same time -- which is what makes a pipe a pipe
 * rather than a temporary file.
 */
static int  msh_pipe_mode = 0;   /* set only inside execute_pipeline */
static int  msh_pipe_out  = -1;  /* this stage's stdout, -1 for the console */
static int  msh_pipe_in   = -1;  /* this stage's stdin,  -1 for the console */
static long msh_stage_pid = 0;   /* what the stage spawned, for the caller */

/*
 * PORT: execvp's job, done by spawn. A bare name means /bin/name; anything
 * containing a slash is taken as written. SYS_SPAWN carries at most 8
 * arguments, and truncating silently would run a DIFFERENT command from the
 * one typed, so that is refused rather than trimmed.
 */
static long msh_spawn_argv(char **v, int outfd, int infd) {
    char path[192];
    long argc = 0;

    while (v[argc]) argc++;
    if (argc > 8) {
        fprintf(stderr, "minishell: %s: more than 8 arguments is not "
                        "supported yet (%ld given)\n", v[0], argc);
        return -1;
    }

    if (strchr(v[0], '/')) snprintf(path, sizeof(path), "%s", v[0]);
    else                   snprintf(path, sizeof(path), "/bin/%s", v[0]);

    return msh_spawn(path, argc, v, outfd, infd);
}
...
static int execute_simple(Simple *s) {
    ...
    /*
     * PORT: fork + redirect + execvp becomes one spawn.
     *
     * The three things the child used to do for itself now happen here,
     * before it exists, because that is where its descriptors are decided.
     * The assignments are applied in the SHELL's environment rather than the
     * child's -- spawn carries no environment block -- so `FOO=1 cmd` sets
     * FOO for this shell as well. That is a real difference from a Unix
     * shell and is the price of having no fork; it is not a bug to be found
     * later.
     */
    {
        int outfd;
        ...
        if (!msh_redirs_to_fds(s->redirs, s->nredir, files, &outfd, &infd)) {
            status = 1;
            goto cleanup;
        }
        ...
        /* An explicit redirection beats the pipe, so `a | b > f` sends b's
           output to the file and not back down the pipe. */
        if (outfd < 0) outfd = msh_pipe_out;
        if (infd  < 0) infd  = msh_pipe_in;

        pid = msh_spawn_argv(args.v, outfd, infd);

        /* Only what THIS call opened. The pipe's ends belong to
           execute_pipeline and are closed there, once every stage has been
           given its copy. */
        if (opened_out >= 0) close(opened_out);
        if (opened_in  >= 0) close(opened_in);

        if (pid <= 0) {
            fprintf(stderr, "minishell: %s: not found\n", args.v[0]);
            status = 127;
        } else if (msh_pipe_mode) {
            msh_stage_pid = pid;      /* the pipeline waits for it */
            status = 0;
        } else {
...
static int execute_pipeline(Pipeline *pl) {
    ...
    for (int i = 0; i < pl->n; i++) {
        ...
        /*
         * PORT: no fork. The stage's descriptors are published here and the
         * ordinary simple-command path spawns with them, without waiting.
         * Every stage is therefore running by the time the loop ends, which
         * is what a pipeline requires -- the writer has to be able to block
         * on a full buffer and have the reader drain it.
         */
        int saved_mode = msh_pipe_mode;
        int saved_out  = msh_pipe_out;
        int saved_in   = msh_pipe_in;

        msh_pipe_mode = 1;
        msh_pipe_out  = has_pipe ? pipefd[1] : -1;
        msh_pipe_in   = (in_fd != 0) ? in_fd : -1;
        msh_stage_pid = 0;

        int stage_status = execute_node(pl->cmds[i]);
        pid_t pid = (pid_t)msh_stage_pid;

        msh_pipe_mode = saved_mode;
        msh_pipe_out  = saved_out;
        msh_pipe_in   = saved_in;

        if (pid <= 0) {
            /* The stage was a builtin, a function or a compound command --
               something that IS the shell rather than a program to start.
               Running it in a pipeline means a second copy of the shell, and
               that is the fork work. Named here so it does not look like the
               command was simply not found. */
            if (stage_status == 0)
                fprintf(stderr, "minishell: only external commands can be "
                                "used in a pipeline in this build\n");
            if (has_pipe) { close(pipefd[0]); close(pipefd[1]); }
            if (in_fd != 0) close(in_fd);
            for (int k = 0; k < i; k++) wait_for_pid(pids[k]);
            free(pids);
            return 1;
        }

        /* The shell's own copies. The child has its own. */
        if (in_fd != 0) close(in_fd);
...
static int execute_list(List *l) {
    ...
    for (int i = 0; i < l->n; i++) {
        ...
        if (l->items[i].bg) {
            /* PORT: `cmd &` backgrounds the SHELL's evaluation of cmd, not a
               program, so it needs a real fork for the same reason $(...)
               does -- there is nothing to exec. */
            pid_t pid = fork();

            if (pid < 0) {
                fprintf(stderr, "minishell: background jobs (&) need fork, "
...
static int execute_subshell(List *l) {
    /* PORT: a subshell is a second copy of the shell, mid-execution, with the
       parent's variables and functions. That is precisely what fork gives you
       and precisely what spawn cannot: there is no program to start. */
    pid_t pid = fork();

    if (pid < 0) {
        fprintf(stderr, "minishell: subshells ( ... ) need fork, which this "
                        "build does not have\n");
        return 1;
    }
